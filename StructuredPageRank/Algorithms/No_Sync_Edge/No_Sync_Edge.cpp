// g++ No_Sync_Edge.cpp -lpthread -o c.out
// ./c.out
...
void* threadPageRank(void* thdnumber) 
{
	...
	//while (iterations[*thdid] < iterationThreshold) 
	{
		threadError[*thdid] = 0;
		for (int u = *thdid + startNode; u <= totalNodes; u = u + numOfThreads) 
		{
			double prev = pageRank[u];
			double sum = 0;
			...
			threadError[*thdid] = max(threadError[*thdid], fabs(prev - pageRank[u]));
		}
...
int main(int argc, char** argv)
{
	...
	file.close();
        ...
        iterations.resize(numOfThreads,0);
	buildLinks();
	...
	pageRank.resize(totalNodes + 1, 1.0000 / (totalNodes + 1));
