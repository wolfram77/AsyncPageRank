		...
		for(int i=*thdid+start_node;i<=nodes;i=i+thdnum) {
			if(red[i]==-1) {
				...
				for(int j=IA[i-1];j<IA[i];j++) {
					//cout<<j<<" ";
					if(red[JA[j]]!=-1) {
						page_rank[JA[j]] = (1-damping_factor)/(nodes+1);
						page_rank[JA[j]] = page_rank[JA[j]] + page_rank_prev[red[JA[j]]]/out_edges[red[JA[j]]] * pow((damping_factor),weight[JA[j]]);
						page_rank[JA[j]] = page_rank[JA[j]] + (damping_factor - pow((damping_factor),weight[JA[j]]))/(nodes+1);
						th_error[*thdid] = max(th_error[*thdid],fabs(page_rank[JA[j]]-page_rank_prev[JA[j]]));
					}
...
int main(int argc, char** argv)
{
	...
	for(int i=Stack.size()-1;i>=0;i--) {
		if(page_rank[Stack[i]]==0) {
			...
			page_rank[j] = page_rank[j] + page_rank_prev[red[j]]/out_edges[red[j]] * pow((damping_factor),weight[j]);
			page_rank[j] = page_rank[j] + (damping_factor - pow((damping_factor),weight[j]))/(nodes+1);
		}
	}
