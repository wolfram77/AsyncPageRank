...
void topoSort(int node) {
	...
	for(int i=0;i<vertexOutLinks[node].size();i++) {
		if(vertexInLinks[vertexOutLinks[node][i]].size()==1&&vertexOutLinks[vertexOutLinks[node][i]].size()==1) {
			if(visited[vertexOutLinks[node][i]]==0) 
				topoSort(vertexOutLinks[node][i]);
		}
	}
...
void chainNodes() {
	for(int i = startNode;i<=totalNodes;i++) {
		if(vertexInLinks[i].size()==1&&vertexOutLinks[i].size()==1) chain.push_back(i);
	}

	//Stack.resize(chain.size(),0);
	visited.resize(maxNodes,0);
	for(int i=0;i<chain.size();i++) {
		if(visited[chain[i]]==0)
			topoSort(chain[i]);
	}

	red.resize(maxNodes,-1);
	weight.resize(maxNodes,-1);

	for(int i=Stack.size()-1;i>=0;i--)
    { 
        int node = Stack[i];
        int c = vertexInLinks[node][0];
        if(vertexInLinks[c].size()==1&&vertexOutLinks[c].size()==1)  {
        	red[node] = red[c];
        	weight[node] = weight[c] + 1;
        }
        else {
        	red[node] = vertexInLinks[node][0];
        	weight[node] = 1;
        }
        //Stack.pop(); 
    }
		...
		for (int u = *thdid + startNode; u <= totalNodes; u = u + numOfThreads) 
		{
			if(red[u]==-1) {
				...
				for (unsigned v = inLinks[u]->start; v <= inLinks[u]->end; v++) {
					...
					int nodeV = vertexInLinks[u][inLinks[u]->start - v];
					if(red[nodeV]!=-1) {
						pageRank[nodeV] = (1-dampingFactor)/(totalNodes+1);
						pageRank[nodeV] = pageRank[nodeV] + contributionList[offsetList[outLinks[red[nodeV]]->start]] * pow((dampingFactor),weight[nodeV]);
						pageRank[nodeV] = pageRank[nodeV] + (dampingFactor - pow((dampingFactor),weight[nodeV]))/(totalNodes+1);
						threadError[*thdid] = max(threadError[*thdid],fabs(pageRank[nodeV]-pageRankPrev[nodeV]));
					}
				}
...
int main(int argc, char** argv)
{
	...
	for(int i=Stack.size()-1;i>=0;i--) {
		if(pageRank[Stack[i]]==0) {
			int j = Stack[i];
			pageRank[j] = (1-dampingFactor)/(totalNodes+1);
			pageRank[j] = pageRank[j] + pageRankPrev[red[j]]/vertexOutLinks[red[j]].size() * pow((dampingFactor),weight[j]);
			pageRank[j] = pageRank[j] + (dampingFactor - pow((dampingFactor),weight[j]))/(totalNodes+1);
		}
	}
